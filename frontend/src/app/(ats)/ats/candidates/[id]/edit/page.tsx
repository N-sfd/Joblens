...
import type { candidate, EmployeeUpdate, EmployeeResume } from "@/types";
...
export default function EditEmployeePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const emp = await api.getCandidate(candidateId);
        setForm(emp);
        setError(null);
        try {
          setResume(await api.getLatestEmployeeResume(candidateId));
        } catch {
  ...
  const update = (field: keyof candidate) => (
  ...
  return (
    <div className="p-4 sm:p-8 max-w-3xl mx-auto">
        ...
        <p className="page-subtitle">
          ...
          {!resume && (
            <span className="block mt-1 text-amber-700">
              ...
              <Link href={`/ats/candidates/${candidateId}`} className="underline font-medium">
                Upload a resume
              </Link>{" "}
      ...
      <div className="flex gap-3 justify-end mt-5">
        <button className="btn-secondary" onClick={() => router.push(`/ats/candidates/${candidateId}`)}>Cancel</button>
