          ...
          {items.map((job) => {
            ...
            return (
              <div key={job.id} className="px-5 py-3 flex items-center gap-3 hover:bg-slate-50 transition-colors">
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${job.reminder_type ? REMINDER_TYPE_COLOR[job.reminder_type] : "bg-slate-100 text-slate-500"}`}>
                  <Icon size={13} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-700 truncate">{job.company} — {job.role}</p>
                  <p className="text-xs text-slate-400">{job.reminder_type ? REMINDER_TYPE_LABEL[job.reminder_type] : "Follow-up"}</p>
                </div>
                <span className={`text-xs font-medium shrink-0 ${overdue ? "text-red-500" : "text-slate-400"}`}>
