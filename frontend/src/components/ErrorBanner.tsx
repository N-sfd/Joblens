...
export default function ErrorBanner({ message, onDismiss, onRetry, className }: Props) {
  return (
    <div className={clsx("flex items-start gap-3 bg-red-50 border border-red-100 rounded-xl px-4 py-3.5", className)}>
      <div className="w-7 h-7 rounded-full bg-red-100 flex items-center justify-center shrink-0 mt-0.5">
        <AlertTriangle size={14} className="text-red-600" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-red-800">Something went wrong</p>
        <p className="text-sm text-red-600/90 mt-0.5 leading-relaxed">{message}</p>
        {onRetry && (
          <button
            type="button"
            onClick={onRetry}
